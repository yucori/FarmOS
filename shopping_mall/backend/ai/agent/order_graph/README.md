...
LangGraph `StateGraph` 기반 취소·교환·주문 변경 멀티스텝 HitL 플로우.
...
취소/교환/주문 변경은 여러 턴에 걸친 대화가 필요합니다.
...
class OrderState(TypedDict):
    ...
    change_type: str | None      # 변경 플로우만
    change_detail: str | None    # 변경 플로우만
...
START
  → route_action
  → list_orders          [interrupt: 주문 선택]
  → get_change_type      [interrupt: 변경 항목 선택]
  → get_change_detail    [interrupt: 현재 등록값 표시 + 변경 내용 입력]
  → show_summary         [interrupt: 최종 확인]
  → create_ticket        → END
```
...
| `get_change_type` | ✓ | 변경 항목 선택 대기 (변경 전용) |
| `get_change_detail` | ✓ | 현재 등록값 표시 후 변경 내용 입력 대기 (변경 전용) |
...
`ShopTicket.flags`는 운영팀이 우선 확인해야 하는 티켓 메타를 JSON 배열 문자열로 저장합니다.
...
- `high_value_review`: `exchange` 티켓이고 `Order.total_price >= 50000`이면 저장
- 고객 응답에는 "주문 금액 기준으로 운영팀이 우선 확인합니다"를 덧붙임
- 어드민 티켓 목록/상세에는 배지와 "운영 확인 항목"으로 표시
